...
function normalizeState(record, readFailed = false) {
  if (readFailed || !record || record._id !== CONFIG_ID || record.schemaVersion !== CONFIG_SCHEMA_VERSION || typeof record.enabled !== 'boolean') {
    return { enabled: true, valid: false, failClosed: true };
  }
  return { enabled: record.enabled, valid: true, failClosed: false };
}

async function assertAvailable(db, businessError) {
  let state;
  try {
    const result = await db.collection(CONFIG_COLLECTION).doc(CONFIG_ID).get();
    state = normalizeState(extractRecord(result));
  } catch (error) {
    state = normalizeState(null, true);
  }
  if (state.enabled) businessError(ERROR_CODE, '服务维护中，请稍后再试');
  return state;
}

module.exports = { CONFIG_COLLECTION, CONFIG_ID, CONFIG_SCHEMA_VERSION, ERROR_CODE, normalizeState, assertAvailable };
