...
第十二至第十七阶段均已完成并以对应的 `phase-12-complete` 至 `phase-17-complete` 标签封版。第十五阶段导入 2952 所官方高校，上海工程技术大学、上海财经大学浙江学院为 `active`，其余 2950 所为 `pending`；第十六阶段完成用户学校归属，第十七阶段完成新商品可信学校绑定。第十八阶段已完成同校市场正式切换并以 `phase-18-complete` 封版；第十九阶段已完成跨校合法 ID 详情只读与新增关系服务端闸门，并以 `phase-19-complete` 封版；第二十阶段已完成服务端 7 天换校冷却与全局学校状态失效，并以 `phase-20-complete` 封版。第二十一阶段的历史收藏、会话、聊天、预约、卖家主页和我的发布学校适配已实现、部署并通过自动回归、零写入生产验证与必要页面人工验收，以 `phase-21-complete` 封版：
...
第十九阶段完整记录见 `docs/phase-19-cross-school-detail-access.md`；第二十阶段记录见 `docs/phase-20-school-change-cooldown.md`；第二十一阶段实现、部署、零写入验证和人工验收记录见 `docs/phase-21-historical-relation-school-adaptation.md`。Phase 21 已完成 Git 封版；这不代表已经通过微信官方审核或正式发布上线。后续阶段尚未开始。
...
客户端仅提交经过校验的商品字段和云文件 `fileID`，不会提交商品学校。卖家、学校、状态、计数及服务端时间均由云函数构造，发布请求 ID 与用户 ID 共同生成确定性商品文档 ID，避免超时重试产生重复商品。新商品的 `schoolId / schoolName` 是发布时快照，普通编辑、上下架和售出不会改变；历史无学校商品不会因编辑而自动补齐。本阶段尚未按学校过滤首页、搜索、分类或详情。
...
验证覆盖 JSON、页面和组件四件套、真实身份唯一性与并发登录、统一云初始化、资料校验、头像安全、登录守卫、商品查询与生命周期、收藏、聊天、预约、媒体和位置边界，以及学校数据、首次选校、可信商品绑定、学校切换、强制登录、同校隔离、四排序 seek 游标、搜索/分类、跨校游标拒绝、迁移、回滚与恢复。当前综合验证为 `81 checks passed`；阶段 16/选校专项为 128 项，阶段 17/商品学校绑定为 51 项，阶段 18 同校市场为 91 项。完整命令和最终证据见 `docs/phase-18-final-cutover.md`。

## 后续阶段

第十五阶段学校基础库、两所真实测试学校激活和只读查询线上闭环已经完成；当前 active 2、pending 2950。第十六阶段源码、自动验证、`authUser` 部署和最终人工验收已经完成。第十七阶段新商品学校可信绑定、历史商品兼容和云端验收已经完成。第十八阶段同校市场现已正式全量切换：市场强制登录、服务端权威学校隔离、四排序稳定游标、搜索/分类、跨校游标拒绝、换校作用域失效、历史关系兼容和公开商品学校迁移均已落地；账号 A/B 真机、第三既有用户、显式退出重启、真实 legacy 回滚及 strict 恢复全部通过。最终配置为 `enabled=true / strictForAll=true / allowlist=[] / accessRequiresAuth=true`，详见 `docs/phase-18-final-cutover.md`。第十九阶段跨校详情与新关系闸门、第二十阶段换校冷却均已封版。第二十一阶段已完成开发、最小云函数部署、卖家主页组合索引、自动回归、零写入生产验证、preview 和必要页面人工验收，并以 `phase-21-complete` 封版。后续阶段尚未开始。
