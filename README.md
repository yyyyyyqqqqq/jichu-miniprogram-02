...
- 真实学校激活、用户选校、商品学校绑定、同校市场隔离、存量迁移及相关 UI（学校基础库和只读服务已经完成）
- 任何在线支付、担保支付或物流能力
...
第十五阶段学校基础库与只读查询已经完成，但 2952 所真实学校仍全部为 `pending`。进入第十六阶段前，应先由产品负责人明确至少两所真实测试学校，通过受控运维操作激活并验证线上 list、search、detail 只返回这些学校；之后阶段 16—25 再依次完成用户选校、商品绑定、同校查询、跨校边界、学校切换、历史关系适配、迁移、安全、UI 和综合验收。
