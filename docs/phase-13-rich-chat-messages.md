...
## 15. 第十三阶段完成后的地图选点与 UI 收尾

2026-07-27，在 `phase-13-complete` 之后完成商品交易地点与三处 UI 的独立收尾。本节只记录阶段十三完成后的完善，不将其命名为第十四阶段。

商品发布与编辑地点改为用户主动点击后打开微信地图选点：

- `pages/publish`、`pages/product-edit` 和既有预约地点页复用 `services/location-service.js`；
- 共享服务统一处理选择成功、用户取消、权限拒绝、字段清理、文本压缩和经纬度范围；
- 取消选点不会误报失败，也不会覆盖编辑页已有地点；
- 新商品必须提交规范化的 `locationDetail`，`cloudfunctions/createProduct` 会严格重新校验名称、地址、坐标和字段；
- 历史商品若只有文本 `location`，仍可正常展示和进入编辑；保持原地点时允许继续保存，但重新选择地点后必须提交合法结构化数据；
- `cloudfunctions/manageProduct` 只在地点发生变化时接受和写入新的结构化地点，继续执行所有权、字段白名单、版本和幂等边界；
- 精确 `locationDetail` 不进入公开商品查询响应，公开页面只使用既有文本地点。

隐私边界保持最小化：项目没有在页面加载时自动获取位置，没有后台或持续定位，也没有注册位置变化监听。只有用户明确点击地点控件时才调用地图选择器；地址提示要求选择校内公共区域，避免宿舍门牌等敏感地点。

本轮同时完成三处已由用户真机确认的视觉收尾：

- 面交预约详情底部操作按钮文字垂直居中；
- 登录资料页“保存资料”按钮文字垂直居中；
- 发布页未选地点时，使用左右对称 Grid 占位列和独立文字容器，使占位文字在箭头存在时仍保持视觉居中。

`createProduct` 与 `manageProduct` 已部署到当前真实环境并反查为 `Active`，云端文件与本地一致。本轮没有创建或修改集合、索引、数据库权限或云存储权限，也不需要再次部署。

最终 `npm run verify` 为 `68 checks passed`，并通过 JavaScript / JSON / WXML / WXSS 静态检查、`git diff --check`、敏感信息扫描和微信开发者工具 CLI 打开编译。用户已在真机完成以下验收并明确确认通过：

- 发布页打开地图、选点、取消、重新选择、发布和详情展示；
- 编辑页读取新结构化地点、兼容旧文本地点、重新选点和保存展示；
- 预约地图选点回归；
- 上述三处按钮或占位文字的垂直居中。
