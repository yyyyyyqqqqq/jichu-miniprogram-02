...
本文件记录源码、自动化验证和已获得的人工验收结论。当前已确认数据库权限、云存储权限、商品组合索引、云函数部署及主要登录、读取和发布流程；跨微信身份删除他人图片的权限拒绝测试明确延期。该项补测及其余发布材料未完成前，不得宣称已经完成全部安全验收或完整具备上线条件。
...
- 剩余验证：真实图片上传和公开读取已通过；使用第二个微信身份删除第一个用户图片的拒绝测试因当前条件不便而延期，发布前必须补测。
...
- 真实查询结果：`authUser`、`productQuery`、`createProduct` 均为 `Active / 3 秒 / Nodejs16.13`。
- 影响：冷启动、组合查询或多次数据库操作可能在平台侧先超时。
- 处理：客户端认证和商品查询超时提高到 15 秒；发布保存保持 15 秒，图片上传 30 秒。
- 待办：在云函数控制台将三个函数的云端超时调整为 10 秒，再做冷启动和弱网回归。CLI 部署参数不能修改超时。

#### SDK 传递依赖和旧运行时

- 三个云函数：`wx-server-sdk@4.0.2`。
...
- 跨微信身份删除他人图片的权限拒绝测试尚未执行，明确延期；
- 延期项必须在体验版发布或宣称完整上线条件前补测并保留结果。
...
```text
productQuery  Active / 3 秒 / Nodejs16.13
createProduct Active / 3 秒 / Nodejs16.13
```

两个云函数的云端下载包与本地 `index.js`、`package.json`、`package-lock.json` 哈希一致。真实调用 `productQuery/seed` 返回 `INVALID_ACTION`，没有重新初始化测试数据。
...
超时调整为 10 秒。保存后重新查询状态并做冷启动、弱网和重复发布测试。
...
24. 用另一微信测试身份尝试删除前一用户的图片，确认权限拒绝。此项当前明确延期，体验版发布前必须补测。
...
跨用户删除拒绝测试尚未完成，因此当前不得把第六阶段描述为“全部安全验收完成”，也不得宣称已经完整具备上线条件。
...
1. 双微信身份跨用户删除图片拒绝测试明确延期，发布前必须补测；
2. 云函数仍为 3 秒超时；
3. 云端仍为 Nodejs16.13；
4. 官方 SDK 的 1 moderate + 5 high 传递依赖风险无兼容稳定修复；
5. 用户生成商品文字和图片没有自动内容审核系统；
6. 服务类目、隐私、协议、联系方式和主体材料尚未确认；
7. 完整双账号、弱网和体验版发布回归尚未全部完成。
...
本次允许提交并推送第六阶段正式代码和本文档，但：

- 不创建 `phase-6-security-release-readiness` 标签；
- 不修改历史；
- 不修改、删除、移动、暂存或提交 `04.md`、`05.md`、`06.md`、`07.md`、`08.md`、`09.md`、`10.md`、`11.md`；
- 跨用户删除拒绝测试完成前，不得通过提交信息、标签或文档宣称第六阶段全部安全验收完成。
